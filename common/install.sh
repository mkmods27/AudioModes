convert_legacy() {
  ui_print "   Old ZIYANSERAudioModules folder detected at: $FOL!"
  ...
  find $FOL/Profile -mindepth 1 -maxdepth 1 -type d 2>/dev/null | while read PROFILE; do
    ...
    for DEVICE in bluetooth headset speaker usb; do
      ...
      for FORMAT in txt xml; do
        if [ "$FORMAT" == "xml" ]; then
          SOURCE="$PROFILE/com.vipercn.ZIYANSERAudioModules_v2.$DEVICE.xml"
        ...
        while read LINE; do
          ...
          if [ "$FORMAT" == "xml" ]; then
            case "$LINE" in
              *"string name="*) VALUE="$(echo "$LINE" | sed -r -e "s|.*>(.*)</.*|\1|" -e "s/FILE://")"; LINE="$(echo "$LINE" | sed -r -e "s|.*name=\"ZIYANSERAudioModules.[A-Za-z]*\.(.*)\".*|\1|" -e "s/viperddc/ddc/" -e "s/\./_/g")";;
              *"boolean name="*) VALUE="$(echo "$LINE" | sed -r "s|.*value=\"(.*)\" />.*|\1|")"; LINE="$(echo "$LINE" | sed -r -e "s|.*name=\"ZIYANSERAudioModules.[A-Za-z]*\.(.*)\" v.*|\1|" -e "s/viperddc/ddc/" -e "s/\./_/g")";;
              *) continue;;
            esac
...
FOL="/storage/emulated/0/ZIYANSERAudioModules"
[ -d "$FOL" ] || mkdir $FOL
...
for i in $(find /data/app -maxdepth 1 -type d -name "*com.pittvandewitt.viperfx*" -o -name "*com.audlabs.viperfx*" -o -name "*com.vipercn.ZIYANSERAudioModules_v2*"); do
  case "$i" in
    ...
    *"com.vipercn.ZIYANSERAudioModules"*) pm uninstall com.vipercn.ZIYANSERAudioModules_v2 >/dev/null 2>&1;;
  esac
done
# Remove remnants of any old v4a installs
for i in $(find /data/data -maxdepth 1 -name "*ZIYANSERAudioModules*" -o -name "*com.audlabs.viperfx*" -o -name "*com.vipercn.ZIYANSERAudioModules_v2*"); do
  rm -rf $i 2>/dev/null
done
...
ui_print "- Installing ZIYANSERAudioModules $(grep_prop version $MODPATH/module.prop)..."
$ENFORCE && setenforce 0
(pm install $MODPATH/v4afx.apk >/dev/null 2>&1) || ui_print " Audio Modes install failed! Install $FOL/v4afx.apk manually"
...
ui_print "   open Audio Modes app and follow the prompts"
ui_print " "
sleep 5

REMS=$(find $NVBASE/modules/*/system $MODULEROOT/*/system -type f -name "ZIYANSERAudioModules.apk" 2>/dev/null)
if [ "$REMS" ]; then
  ui_print "- Marking all old Audio Modes modules for deletion..."
  for i in ${REMS}; do
